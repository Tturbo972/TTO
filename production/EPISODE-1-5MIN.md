# ÉPISODE 1 — « 13 MÈTRES » · VERSION 5 MINUTES
## 9:16 vertical · 30 plans · production hybride

Remplace la version 2:00. Format vertical conservé. **Les 30 plans disposent désormais d'un prompt de mouvement** — animez-les tous si l'offre illimitée couvre Seedance, ou les 9 marqués 🎬 sinon.

⚠️ **Une vidéo verticale de 5 minutes n'est pas un Short** — YouTube plafonne les Shorts à 3 minutes. Celle-ci partira en long-form : plein écran sur mobile, bandes noires sur ordinateur. La vignette reste en 16:9 quoi qu'il arrive.

---

## CE QUI CHANGE PAR RAPPORT À LA VERSION 2:00

Passer de 2 à 5 minutes ne consiste pas à étirer le même récit. Trois faits nouveaux ont été vérifiés en recherche, et chacun est un **retournement**, pas du remplissage :

**Astley n'a jamais appelé ça un « cirque ».** Le mot vient de Charles Hughes, son ancien employé devenu rival, qui ouvre le Royal Circus en 1782 — quatorze ans après. L'inventeur n'a pas nommé son invention.

**Léotard s'entraînait au-dessus d'une piscine.** Son père tenait un gymnase et un bassin à Toulouse. Il a suspendu une barre au-dessus de l'eau : tomber ne coûtait rien. Et il étudiait le droit avant de tout quitter à dix-huit ans.

**Le Cirque du Soleil est né le 16 juin 1984 à Baie-Saint-Paul**, fondé par deux artistes de rue, Guy Laliberté et Gilles Ste-Croix. Sans bêtes, sans vedettes, sans paroles. Ce point était marqué « non vérifié » dans l'ancien dossier — il est confirmé.

La structure passe donc d'**un** renversement à **trois**, ce qui est précisément ce qui permet de tenir cinq minutes sans perdre le spectateur.

---

## SCRIPT VOIX OFF — 712 mots · narration 0:16 → 5:02

Débits vérifiés par acte, tous entre 120 et 159 mots/minute — dans la bande lisible (140-160). Les sous-titres correspondants sont dans `13-metres-5min.srt`, 142 segments.

**Structure temporelle finale :**

| | |
|---|---|
| Hook TTO | 0:00 → 0:16 |
| Narration | 0:16 → 5:02 |
| Piste vide en silence | 5:02 → 5:12 |
| **Durée totale** | **5:12** |

Les dix dernières secondes sont muettes, sur le plan de bouclage. Ce n'est pas du remplissage : c'est le coussin qui permet à l'écran de fin de s'installer **sans recouvrir la chute**, et c'est le silence qui fait remonter le spectateur au début.

### HOOK TTO — 0:00 → 0:16
*(bloc fixe, voir `tto-charte-hook.md`)*

> **① Phrase choc, voix grave :** Toutes les pistes de cirque du monde font exactement treize mètres.
> **⑤ Phrase réponse, voix normale :** Et ce chiffre vient d'un sergent de cavalerie.

---

### ACTE 1 — Le sergent — 0:16 → 1:15

> Philip Astley a vingt-six ans et il sort de la cavalerie. Sergent-major. Il sait une chose que personne d'autre ne sait faire aussi bien : tenir debout sur un cheval au galop.
>
> En 1768, il loue un terrain vague à Londres, entre Westminster Bridge et Blackfriars Road. Il plante des piquets, tend une corde, et trace un cercle dans la terre.
>
> Le cercle n'est pas une décoration. C'est un calcul.
>
> En galopant en cercle, la force centrifuge plaque le cavalier vers l'extérieur et l'aide à tenir en équilibre. En ligne droite, le même exercice est impossible.
>
> Son premier cercle fait dix-neuf mètres. Trop grand : le cheval ralentit, la force retombe, l'homme tombe.
>
> Alors il resserre. Quarante-deux pieds. Treize mètres.
>
> Pourquoi exactement là ? Parce que c'est la portée de son fouet depuis le centre. Debout au milieu, bras tendu, il touche le bord. Il ne mesure pas la piste — il mesure son propre bras.

### ACTE 2 — Le mot qu'il n'a jamais dit — 1:15 → 2:10

> Très vite, Astley comprend un problème d'exploitation : un spectacle de chevaux ne remplit pas une soirée. Les bêtes fatiguent.
>
> Alors il engage des musiciens, des clowns, des acrobates. Des gens dont le métier est d'occuper le public pendant que les chevaux se reposent.
>
> Le cirque moderne vient de naître. Et l'acrobate y est un bouche-trou.
>
> Il manque encore une chose : le nom.
>
> Quatorze ans plus tard, en 1782, un ancien employé d'Astley devenu son rival ouvre un établissement concurrent, à quelques rues de là. Il l'appelle le Royal Circus.
>
> Charles Hughes. C'est lui qui donne son nom au cirque. Pas Astley.
>
> Et « circus », en latin, ne veut pas dire spectacle. Le mot vient du grec *kirkos* : cercle.
>
> Le nom de cet art ne décrit ni les clowns, ni les fauves, ni les acrobates. Il décrit la piste. Il décrit les treize mètres.

### ACTE 3 — L'exportation — 2:10 → 2:55

> Le format se copie. Il traverse la Manche, puis l'Atlantique.
>
> Le chapiteau apparaît, parce qu'il faut pouvoir démonter. L'affiche apparaît, parce qu'il faut annoncer avant d'arriver. Le train devient l'outil du cirque américain : on décharge, on monte, on joue, on démonte, on repart.
>
> Tout devient mobile. Les toiles, les mâts, les gradins, les animaux, les artistes.
>
> Tout, sauf une chose.
>
> Partout où le cirque s'installe — un pré anglais, une plaine du Kansas, une place italienne — quelqu'un descend du train, plante un piquet, et retrace le même cercle. Treize mètres.
>
> Parce qu'un numéro équestre répété mille fois ne fonctionne que sur ce diamètre-là. Changer la piste obligerait à réapprendre le métier.

### ACTE 4 — Le garçon au-dessus de la piscine — 2:55 → 4:10

> Toulouse, années 1840. Un homme tient un gymnase, et avec le gymnase, un bassin de natation. Il a un fils.
>
> Le père suspend une barre au-dessus de l'eau. L'idée est simple : au-dessus d'une piscine, tomber ne coûte rien. On recommence.
>
> Le fils s'entraîne là pendant des années. Il étudie le droit en parallèle. À dix-huit ans, il abandonne le droit.
>
> 12 novembre 1859. Cirque Napoléon, Paris. Il a vingt-et-un ans.
>
> Il s'élance d'une plateforme, lâche sa barre, et attrape la suivante en plein vol.
>
> Personne n'a jamais fait ça. Napoléon III vient le voir.
>
> Pour son numéro, il a dessiné un vêtement moulant d'une seule pièce, qui ne gêne pas les épaules. Il l'appelle simplement « le maillot ».
>
> En 1867, les Anglais lui écrivent une chanson : *The Daring Young Man on the Flying Trapeze*. Le garçon de la piscine est devenu une célébrité européenne.
>
> Le 16 août 1870, il meurt à Toulouse. Il a trente-deux ans.
>
> Et c'est après sa mort que les Anglais donnent son nom au vêtement. Il s'appelait Jules Léotard.

### ACTE 5 — Le renversement — 4:10 → 4:50

> Un siècle passe. Les numéros d'animaux sont contestés, puis restreints, puis interdits dans une grande partie du monde.
>
> Le 16 juin 1984, à Baie-Saint-Paul, au Québec, deux artistes de rue montent une troupe. Guy Laliberté et Gilles Ste-Croix.
>
> Leur spectacle n'a pas une seule bête. Pas de vedette non plus. Pas un mot prononcé.
>
> Rien que des corps humains, sur une piste ronde.
>
> Le bouche-trou de 1768 — celui qu'on engageait pour meubler pendant que les chevaux soufflaient — est devenu le spectacle tout entier.

### CHUTE — 4:50 → 5:02

> Le nom est venu d'un rival. Les chevaux sont partis. Le bouche-trou est devenu la vedette.
>
> Tout a changé. Sauf les treize mètres.
>
> Et toi, tu l'avais remarqué ?

---

## SHOT LIST — 30 plans

`🎬` = animé en Seedance 4K · `♻️` = réutilise le clip du plan 1

**Offre illimitée confirmée en 9:16 4K — les 30 plans sont animés.** Seules les images sources restent payantes.

| # | Time | Durée | Sujet | Texte écran | Mode |
|---|---|---|---|---|---|
| 1 | 0:16 | 10 s | Piste vide vue du dessus | | 🎬 |
| 2 | 0:26 | 9 s | Silhouette d'officier dans une porte d'écurie | **1768** | 🎬 |
| 3 | 0:35 | 9 s | Terrain vague, piquets et corde tendue | *Londres* | 🎬 |
| 4 | 0:44 | 10 s | Main plantant un piquet dans la terre | | 🎬 |
| 5 | 0:54 | 11 s | Cavalier debout sur cheval au galop | | 🎬 |
| 6 | 1:05 | 10 s | Sabots, arc de terre projetée | | 🎬 |
| 7 | 1:15 | 10 s | Fouet se déroulant depuis le centre | *13 m* | 🎬 |
| 8 | 1:25 | 10 s | Galeries se remplissant de foule | | 🎬 |
| 9 | 1:35 | 10 s | Musiciens et clown entrant en piste | | 🎬 |
| 10 | 1:45 | 10 s | Chevaux à l'écurie, au repos | | 🎬 |
| 11 | 1:55 | 10 s | Façade rivale, enseigne gravée | **1782** | 🎬 |
| 12 | 2:05 | 10 s | Le mot ROYAL CIRCUS gravé | **ROYAL CIRCUS** | 🎬 |
| 13 | 2:15 | 10 s | Cercle tracé au compas sur un plan | *kirkos = cercle* | 🎬 |
| 14 | 2:25 | 10 s | Chapiteau monté sur un pré | | 🎬 |
| 15 | 2:35 | 10 s | Affiches placardées sur un mur | | 🎬 |
| 16 | 2:45 | 10 s | Train de cirque à l'arrêt, wagons | | 🎬 |
| 17 | 2:55 | 10 s | Piquet planté, cercle retracé | **13 m — PARTOUT** | 🎬 |
| 18 | 3:05 | 10 s | Gymnase toulousain, agrès | *Toulouse* | 🎬 |
| 19 | 3:15 | 11 s | Barre suspendue au-dessus d'un bassin | | 🎬 |
| 20 | 3:26 | 9 s | Surface de l'eau, reflet de la barre | | 🎬 |
| 21 | 3:35 | 10 s | Livres de droit fermés, poussière | | 🎬 |
| 22 | 3:45 | 10 s | Intérieur du Cirque Napoléon | **12 NOVEMBRE 1859** | 🎬 |
| 23 | 3:55 | 10 s | Mains craie sur barre de trapèze | | 🎬 |
| 24 | 4:05 | 11 s | Corps en vol entre deux barres | **IL LÂCHE LA BARRE** | 🎬 ⭐ |
| 25 | 4:16 | 9 s | Le maillot posé à plat, vu du dessus | **« LE MAILLOT »** | 🎬 |
| 26 | 4:25 | 10 s | Partition et affiche de music-hall | **1867** | 🎬 |
| 27 | 4:35 | 8 s | Pierre tombale toulousaine, sobre | **1870 — 32 ANS** | 🎬 |
| 28 | 4:43 | 9 s | Cage vide, porte ouverte | | 🎬 |
| 29 | 4:52 | 10 s | Acrobate contemporain sur tissus | **1984** | 🎬 |
| 30 | 5:02 | 10 s | Piste vide — **cadrage identique au plan 1** | **SAUF LES 13 MÈTRES** | ♻️ plan 1 |

**29 images à générer × 2 = 58 crédits.**
**30 plans animés = 0 crédit** (offre illimitée, 9:16 4K).
**Total 58 crédits** sur 356 — il reste **298 crédits**.

Les images sources restent payantes : chaque plan animé part d'une image fixe validée. C'est le seul poste du film.

### Ce que la gratuité ne change pas

Les 9 plans ci-dessous restaient les plus importants quand il fallait choisir. Ils le restent : ce sont eux qu'il faut regarder en premier au contrôle, parce que **le mouvement y est le sujet** et qu'un défaut s'y verra immédiatement.

- **1 et 30** — l'ouverture et le bouclage. Ce sont les deux plans que le spectateur regarde le plus longtemps.
- **5** — le galop, seul plan où le mouvement porte le fait expliqué.
- **7** — le fouet qui se déroule *est* la mesure des treize mètres.
- **13** — le compas qui trace le cercle, cœur visuel de l'acte 2.
- **16** — le train, symbole de l'exportation.
- **19** — la barre au-dessus de l'eau, image la plus mémorable de l'acte 4.
- **22 et 24** — le Cirque Napoléon et le corps en vol. Le plan 24 est le sommet du film.

Les 21 autres sont désormais animés eux aussi, mais leur mouvement est décoratif : s'ils tremblent, le repli sur l'image fixe avec un zoom lent de 3 % ne coûte rien et personne ne verra la différence.

---

## OFFRE ILLIMITÉE SEEDANCE — À VÉRIFIER AVANT DE LANCER

Si votre compte dispose de générations illimitées sur Seedance, le budget de ce dossier s'effondre et **les 30 plans peuvent être animés**, pas seulement 9.

Une réserve, et une seule : l'allocation illimitée ne couvre pas le modèle en bloc, elle couvre **une liste de configurations précises**. Résolution, format et durée en font partie. Il est courant qu'une offre d'essai couvre le 1080p et exclue la 4K, ou couvre le 16:9 et pas le vertical.

### La vérification, en une commande

Dans l'application ou via l'API, appelez l'explorateur de modèles avec le filtre illimité :

```
models_explore(action: "list", type: "video", unlim: true)
```

Trois choses à lire dans la réponse :

1. **`seedance` figure-t-il dans la liste ?** S'il n'y est pas, l'offre ne le couvre pas.
2. **Le bloc `unlim` en tête** dit si vous pouvez dépenser ces générations *maintenant* — une allocation peut exister mais être épuisée ou expirée.
3. **La ligne « Unlim configs » en fin de réponse** énumère les configurations réellement couvertes. C'est la seule qui compte : cherchez-y `9:16` **et** `4k`. Si vous n'y trouvez que `1080p`, l'illimité fonctionne quand même — voir le scénario B.

Vérifiez aussi qu'aucun filigrane n'est appliqué aux générations gratuites. Un filigrane sur trente plans rendrait le film inutilisable.

### Les quatre scénarios chiffrés

| | Situation | Coût | Reste | Film |
|---|---|---|---|---|
| **A** | Illimité couvre 9:16 en 4K | **58 cr.** | 298 | 5:12, **30 plans animés** |
| **B** | Illimité, mais 1080p seulement | **58 cr.** | 298 | 5:12, **30 plans animés** |
| **C** | Pas d'illimité — plan actuel | 260 cr. | 96 | 5:12, 9 plans animés |
| **D** | Pas d'illimité — 2:00 tout animé | **343 cr.** | **13** | 2:00, 14 plans animés |

Dans les scénarios A et B, les 58 crédits ne paient que les **images sources** — chaque plan animé part d'une image fixe validée, et celle-là reste payante.

### Le scénario B n'est pas une déception

Si l'illimité ne couvre que le 1080p, prenez-le sans hésiter. C'est déjà l'argument développé dans `seedance-animation.md` : YouTube rediffuse en 1080 × 1920 quoi qu'il arrive, et agrandir une gravure ne crée aucun détail — la finesse vient du trait, déjà présent dans l'image source. La 4K n'aurait servi qu'à conserver un master.

**Animer trente plans en 1080p vaut infiniment mieux qu'en animer neuf en 4K.**

### ⚠️ Réduire à 2 minutes est le mauvais levier

Le repli proposé — raccourcir le film pour financer une animation intégrale — coûte **plus cher** que le plan actuel :

**343 crédits pour 2 minutes, contre 260 pour 5:12.** Vous paieriez 83 crédits de plus pour un film deux fois et demie plus court, et il ne resterait que 13 crédits : **pas une seule reprise possible**. Un plan qui tremble, et vous êtes bloqué.

La raison est arithmétique : raccourcir le film réduit le nombre de plans, mais passer de 9 à 14 plans *animés* augmente la dépense bien plus vite que la durée ne la diminue. Le coût suit les plans animés, pas les minutes.

**Si l'illimité ne fonctionne pas, gardez les 5 minutes en hybride** (scénario C) : plus long, moins cher, et 96 crédits de marge. Ou, à budget serré, une version 2:00 hybride à 5 plans animés — 140 crédits, 216 de reste.

### Si l'illimité fonctionne, ce qui change dans ce dossier

La colonne « Mode » de la shot list devient caduque : **les 30 plans passent en 🎬**. Les prompts de mouvement des 9 plans sélectionnés sont déjà écrits ; il en manque 21, que je rédige dès que vous confirmez.

Le principe directeur ne change pas et reste vital : **le mouvement vient de la caméra, jamais du sujet.** Une animation gratuite ne protège pas la gravure du tremblement — elle rend seulement les reprises indolores. Testez toujours sur le plan 24 avant de lancer la série.

---

## PROMPTS — les 16 nouveaux plans

Les plans 1, 2, 5, 6, 7, 8, 9, 22, 23, 24, 25, 29 et 30 réutilisent les prompts déjà écrits dans `prompts-a-coller.txt`. Voici les seize nouveaux.

Bloc de style identique partout — ne le modifiez jamais.

**Plan 3 — terrain vague**
> A 19th-century lithographic engraving, tall vertical composition: an empty patch of waste ground on the edge of Georgian London, wooden stakes driven into the earth and a rope stretched between them marking out a wide circle, rooftops low on the horizon. Etched line work and dense cross-hatching, aged paper texture with visible fibre and foxing, limited palette of ink black, warm sepia and a single crimson accent, subtle paper grain. No text, no captions, no watermark.

**Plan 4 — le piquet**
> A 19th-century lithographic engraving, vertical composition: an extreme close-up of a bare hand driving a wooden stake into hard earth with a mallet, soil breaking around the point, strong directional light. Etched line work and dense cross-hatching, aged paper texture with visible fibre and foxing, limited palette of ink black, warm sepia and a single crimson accent, subtle paper grain. No text, no captions, no watermark.

**Plan 10 — les chevaux au repos**
> A 19th-century lithographic engraving, tall vertical composition: three horses standing at rest in a dim stable, heads lowered, steam rising from their flanks, a shaft of light from a high window above them. Etched line work and dense cross-hatching, aged paper texture with visible fibre and foxing, limited palette of ink black, warm sepia and a single crimson accent, subtle paper grain. No text, no captions, no watermark.

**Plan 11 — la façade rivale**
> A 19th-century lithographic engraving, tall vertical composition: the tall facade of a rival Georgian playhouse at dusk, carriages waiting outside, lit windows stacked up the height of the frame, a crowd in silhouette at the doors. Etched line work and dense cross-hatching, aged paper texture with visible fibre and foxing, limited palette of ink black, warm sepia and a single crimson accent, subtle paper grain. No text, no captions, no watermark.

**Plan 12 — l'enseigne gravée**
> A 19th-century lithographic engraving, vertical composition: an extreme close-up of deeply carved wooden signboard lettering, raking light across the cut grooves, worn paint in the letterforms, shallow depth of field. Etched line work and dense cross-hatching, aged paper texture with visible fibre and foxing, limited palette of ink black, warm sepia and a single crimson accent, subtle paper grain. No readable words, no text, no captions, no watermark.

**Plan 13 — le compas sur le plan**
> A 19th-century lithographic engraving, vertical composition: a pair of brass dividers standing on an old architectural drawing, one point pinned at the centre while the other scribes a perfect circle across the paper, faint construction lines visible. Etched line work and dense cross-hatching, aged paper texture with visible fibre and foxing, limited palette of ink black, warm sepia and a single crimson accent, subtle paper grain. No text, no captions, no watermark.

**Plan 14 — le chapiteau**
> A 19th-century lithographic engraving, tall vertical composition: a large canvas circus tent freshly raised in an open field, its central mast rising the full height of the frame, guy ropes radiating outward, low horizon. Etched line work and dense cross-hatching, aged paper texture with visible fibre and foxing, limited palette of ink black, warm sepia and a single crimson accent, subtle paper grain. No text, no captions, no watermark.

**Plan 15 — les affiches**
> A 19th-century lithographic engraving, vertical composition: a brick wall densely covered in layered pasted playbills, edges peeling and overlapping, seen straight on, weathered and torn. Etched line work and dense cross-hatching, aged paper texture with visible fibre and foxing, limited palette of ink black, warm sepia and a single crimson accent, subtle paper grain. No readable words, no text, no captions, no watermark.

**Plan 16 — le train de cirque**
> A 19th-century lithographic engraving, tall vertical composition: a long circus train halted on open plain track at dawn, flat wagons loaded with canvas and poles, steam rising vertically from the locomotive, seen from low beside the rails. Etched line work and dense cross-hatching, aged paper texture with visible fibre and foxing, limited palette of ink black, warm sepia and a single crimson accent, subtle paper grain. No text, no captions, no watermark.

**Plan 17 — le cercle retracé**
> A 19th-century lithographic engraving, vertical composition: a rope stretched taut from a central stake, scoring a fresh circular line into open ground, the arc curving away into the distance, grass flattened along its path. Etched line work and dense cross-hatching, aged paper texture with visible fibre and foxing, limited palette of ink black, warm sepia and a single crimson accent, subtle paper grain. No text, no captions, no watermark.

**Plan 18 — le gymnase toulousain**
> A 19th-century lithographic engraving, tall vertical composition: the interior of a provincial French gymnasium, wooden parallel bars and hanging rings, tall shuttered windows letting in bars of light, empty and still. Etched line work and dense cross-hatching, aged paper texture with visible fibre and foxing, limited palette of ink black, warm sepia and a single crimson accent, subtle paper grain. No text, no captions, no watermark.

**Plan 19 — la barre au-dessus du bassin** ⭐
> A 19th-century lithographic engraving, tall vertical composition: a wooden trapeze bar suspended on ropes directly above the still surface of an indoor swimming pool, the ropes rising out of the top of the frame, water filling the lower half, light reflecting off the surface. Etched line work and dense cross-hatching, aged paper texture with visible fibre and foxing, limited palette of ink black, warm sepia and a single crimson accent, subtle paper grain. No text, no captions, no watermark.

**Plan 20 — la surface de l'eau**
> A 19th-century lithographic engraving, vertical composition: an extreme close-up of a still indoor water surface, the reflection of a suspended wooden bar and its ropes broken by faint ripples, dark depths below. Etched line work and dense cross-hatching, aged paper texture with visible fibre and foxing, limited palette of ink black, warm sepia and a single crimson accent, subtle paper grain. No text, no captions, no watermark.

**Plan 21 — les livres de droit**
> A 19th-century lithographic engraving, vertical composition: a stack of heavy closed law books on a wooden desk, a layer of dust on the top cover, side light raking across the leather spines, abandoned. Etched line work and dense cross-hatching, aged paper texture with visible fibre and foxing, limited palette of ink black, warm sepia and a single crimson accent, subtle paper grain. No readable words, no text, no captions, no watermark.

**Plan 26 — la partition**
> A 19th-century lithographic engraving, vertical composition: an old music-hall song sheet lying on a table beside a rolled playbill, engraved staves and notes visible, gaslight falling across the paper. Etched line work and dense cross-hatching, aged paper texture with visible fibre and foxing, limited palette of ink black, warm sepia and a single crimson accent, subtle paper grain. No readable words, no text, no captions, no watermark.

**Plan 27 — la pierre tombale**
> A 19th-century lithographic engraving, tall vertical composition: a plain weathered headstone standing alone in a southern French cemetery, cypress trees behind it, long evening shadows, no legible inscription. Etched line work and dense cross-hatching, aged paper texture with visible fibre and foxing, limited palette of ink black, warm sepia and a single crimson accent, subtle paper grain. No readable words, no text, no captions, no watermark.

**Plan 28 — la cage vide**
> A 19th-century lithographic engraving, tall vertical composition: an empty iron menagerie cage with its door standing open, straw scattered on the floor, seen from slightly below, darkness beyond the bars. Etched line work and dense cross-hatching, aged paper texture with visible fibre and foxing, limited palette of ink black, warm sepia and a single crimson accent, subtle paper grain. No text, no captions, no watermark.

⚠️ **Plans 12, 15, 21, 26, 27 :** ces images comportent des surfaces où le modèle voudra écrire du texte — enseignes, affiches, livres, partitions, pierre tombale. Le `No readable words` est là pour ça. Si des lettres apparaissent quand même, elles seront inventées et fausses : régénérez, ou cadrez plus serré au montage.

---

## LES 9 PROMPTS DE MOUVEMENT

Image fixe validée en entrée, ce texte en prompt de mouvement. `seedance_2_0`, `9:16`, **1080p**, clips de 10 s.

Bloc négatif commun à coller dans chacun :
> no morphing, no warping, no melting, no flickering lines, no shimmering texture, no style drift, no added detail, no photorealism, no colour shift, no text, no captions, no watermark, no face deformation, preserve the etched line work and paper grain exactly.

**Plan 1 — la piste vide**
> MOTION: extremely slow push-in toward the centre of the ring, barely perceptible. The engraving itself is completely static. Only fine dust motes drift slowly through the air. Locked, steady, no rotation.

**Plan 2 — la silhouette**
> MOTION: the officer stands perfectly still, absolutely no body movement. Straw dust and fine particles drift slowly downward through the shaft of light. Extremely slow push-in toward the doorway.

**Plan 5 — le cavalier**
> MOTION: the horse and rider are frozen mid-stride, a static engraved tableau — no leg movement, no gait animation whatsoever. Only the mane, the coat tails and airborne dust drift. Slow camera push-in from below.

**Plan 7 — le fouet**
> MOTION: slow camera travel forward along the line of the whip, following it into depth toward the distant ring edge. The whip itself stays perfectly still on the ground. Fine sawdust drifts.

**Plan 13 — le compas**
> MOTION: the dividers stay perfectly still, pinned at the centre. Only the scribed circular line grows slowly across the paper, and fine paper dust drifts in the light. Very slow push-in on the pivot point.

**Plan 16 — le train**
> MOTION: the train is completely stationary, no wheel movement, no travel. Only the steam rises slowly and vertically from the locomotive, drifting sideways at the top of the frame. Extremely slow push-in along the rails.

**Plan 19 — la barre au-dessus du bassin**
> MOTION: the suspended bar hangs perfectly still. Only the water surface below moves — very slow, gentle ripples spreading outward, light shifting on the surface. Slow descending camera move toward the water.

**Plan 22 — le Cirque Napoléon**
> MOTION: slow tilt upward across the stacked balconies. The crowd stays a static silhouette mass, no individual movement. Chandelier light flickers faintly, haze drifts in the upper air.

**Plan 24 — le corps en vol** ⭐
> MOTION: the body hangs suspended in the air in extreme slow motion, drifting almost imperceptibly, arms held still. The shaft of light breathes faintly. No limb articulation, no rotation. Slow, weightless, held.

**Plan 30** — réutiliser le clip du plan 1, ralenti pour atteindre la durée. Aucun crédit.

---

## LES 20 PROMPTS DE MOUVEMENT RESTANTS

Complètent les 9 déjà écrits. Avec eux, **les 30 plans sont animables** — le plan 30 réutilisant le clip du plan 1, il n'en faut pas de trente-et-unième.

Même bloc négatif commun que les autres, à coller dans chacun :
> no morphing, no warping, no melting, no flickering lines, no shimmering texture, no style drift, no added detail, no photorealism, no colour shift, no text, no captions, no watermark, no face deformation, preserve the etched line work and paper grain exactly.

Le principe ne change pas : **la caméra bouge, le sujet non.** Ce qui vit dans l'image, ce sont les éléments sans contour net — poussière, vapeur, fumée, lumière, eau.

**Plan 3 — terrain vague**
> MOTION: the ground, stakes and rope are completely static. Only fine dust and loose grass drift low across the earth. Slow camera push-in along the line of the rope toward the far side of the circle.

**Plan 4 — la main et le piquet** ⚠️ *risque élevé*
> MOTION: the hand and mallet are frozen at the moment of impact, no arm movement, no swing, no articulation. Only loose soil particles scatter and fall slowly around the point of the stake. Extremely slow push-in.

**Plan 6 — les sabots**
> MOTION: clods of earth and dust hang and drift slowly upward in the air, as if in slow motion. The hooves themselves stay frozen mid-stride. Camera locked low and still.

**Plan 8 — les galeries**
> MOTION: the crowd is a single static silhouette mass — no individual figures move, no heads turn. Only the gaslight glow pulses gently and haze drifts. Very slow tilt upward across the tiers.

**Plan 9 — musiciens et clown**
> MOTION: the figures are frozen mid-step from behind, no walking animation, no limb movement. Only the suspended dust in the light beams drifts. Slow camera push-in following behind them.

**Plan 10 — les chevaux au repos**
> MOTION: the horses stand perfectly still, heads lowered, no movement of legs or heads. Only the steam rises slowly from their flanks and dust drifts through the shaft of light above. Extremely slow push-in.

**Plan 11 — la façade rivale**
> MOTION: the building and the waiting carriages are completely static. Only the lamplight in the windows flickers faintly and evening haze drifts across the facade. Slow push-in with a very slight tilt upward.

**Plan 12 — l'enseigne gravée**
> MOTION: the carved wood is entirely static, no movement at all. Only the raking light shifts very slowly across the cut grooves, deepening and releasing the shadows. Extremely slow lateral drift.

**Plan 14 — le chapiteau**
> MOTION: the tent, mast and guy ropes are static. Only the canvas edges breathe faintly in the wind and clouds drift slowly behind the mast. Slow push-in toward the entrance.

**Plan 15 — les affiches**
> MOTION: the wall and the pasted bills are static. Only the torn edges of the topmost posters lift and settle faintly, and fine dust drifts. Slow lateral camera drift across the wall.

**Plan 17 — le cercle retracé**
> MOTION: the rope stays taut and perfectly still. Only the scored line grows slowly along the ground ahead of it, and loose earth drifts. Slow camera follow along the curve of the arc.

**Plan 18 — le gymnase toulousain**
> MOTION: the room is empty and still, the bars and rings barely swaying, almost imperceptibly. Dust motes drift slowly through the bars of light falling from the shuttered windows. Slow push-in down the length of the hall.

**Plan 20 — la surface de l'eau** ⭐ *le seul plan où le sujet doit bouger*
> MOTION: slow, gentle ripples spread outward across the still water surface, the reflection of the bar and its ropes breaking and reforming. Light shifts on the moving surface. Camera locked and still, no push-in.

**Plan 21 — les livres de droit**
> MOTION: the books are completely static, nothing moves or opens. Only dust motes drift and settle slowly in the raking side light. Extremely slow push-in on the top cover.

**Plan 23 — les mains sur la barre**
> MOTION: the hands stay locked in their grip, no finger movement. Fine chalk dust falls slowly from the bar. Extremely slow push-in on the knuckles.

**Plan 25 — le maillot**
> MOTION: the fabric is completely still, no rippling, no folding. Only the raking light moves gently across the knit texture. Slow overhead camera drift along the length of the garment.

**Plan 26 — la partition**
> MOTION: the paper lies completely still, no pages turning, no curling. Only the gaslight flickers faintly across the sheet and fine dust drifts above it. Slow push-in.

**Plan 27 — la pierre tombale**
> MOTION: the headstone is entirely static. Only the cypress branches behind it move very faintly, and the long evening shadows lengthen almost imperceptibly across the ground. Very slow push-in.

**Plan 28 — la cage vide**
> MOTION: the cage and its open door are completely static, no swinging. Only loose straw dust drifts across the floor and light shifts slowly through the bars. Slow push-in through the open doorway into the darkness beyond.

**Plan 29 — l'acrobate sur tissus**
> MOTION: the silks ripple very slowly along their length. The acrobat's silhouette stays still, no limb movement, no spinning. Slow descending camera move. Single beam of light breathing faintly.

### Deux plans à surveiller

**Le plan 4 est le plus risqué de tout le film.** Une main tenant un maillet est exactement ce qu'un modèle vidéo déforme : les doigts fusionnent, l'outil se tord. Le prompt le fige entièrement et ne laisse vivre que la terre projetée. Si le rendu tremble malgré tout, gardez l'image fixe — un zoom lent sur une main figée au moment de l'impact est même plus fort qu'un mouvement approximatif.

**Le plan 20 est l'exception qui confirme la règle.** C'est le seul où le sujet lui-même doit bouger, et c'est sans danger : l'eau n'a pas de contour fixe, donc rien ne peut s'y déformer visiblement. C'est aussi pour ça que ce plan sera probablement le plus réussi du film — profitez-en, c'est le seul mouvement franc que le style autorise.

---

## SOURCES

- Philip Astley, 1768, terrain entre Westminster Bridge et Blackfriars Road ; premier cercle ~62 pieds (19 m) puis adoption du standard de 42 pieds (13 m) — [Britannica](https://www.britannica.com/art/circus-theatrical-entertainment/Philip-Astley-and-the-first-circuses), [Circopedia](https://www.circopedia.org/Philip_Astley)
- Charles Hughes, Royal Circus, 1782, première utilisation du mot « circus » ; du latin *circus*, du grec *kirkos* = cercle — [Circus250](https://circus250.com/circus250-archive/history), [Wikipedia](https://en.wikipedia.org/wiki/Philip_Astley)
- Jules Léotard, né 1838 à Toulouse, formé par son père professeur de gymnastique tenant aussi une piscine, entraînement au trapèze au-dessus du bassin, études de droit abandonnées à 18 ans — [BnF](https://expositions.bnf.fr/cnac/grand/cir_2014.htm), [Wikipedia](https://en.wikipedia.org/wiki/Jules_L%C3%A9otard)
- Premier trapèze volant le 12 novembre 1859 au Cirque Napoléon (Victor Franconi) — [MoneyWeek](https://moneyweek.com/355915/12-november-1859-jules-leotards-flying-trapeze)
- Chanson *The Daring Young Man on the Flying Trapeze*, 1867, George Leybourne ; mort le 16 août 1870 à Toulouse à 32 ans — [Wikipedia](https://en.wikipedia.org/wiki/Jules_L%C3%A9otard)
- Cirque du Soleil fondé le 16 juin 1984 à Baie-Saint-Paul par Guy Laliberté et Gilles Ste-Croix, sans animaux, sans vedettes, sans dialogue — [The Canadian Encyclopedia](https://www.thecanadianencyclopedia.ca/en/article/cirque-du-soleil), [Wikipedia](https://en.wikipedia.org/wiki/Cirque_du_Soleil)

**Reste non vérifié :** l'affirmation selon laquelle le mot « leotard » n'a été appliqué au vêtement qu'**après** la mort de Léotard. Les sources confirment qu'il a créé et popularisé le vêtement et qu'il porte son nom, mais aucune ne date précisément le basculement. La formulation du script — « c'est après sa mort que les Anglais donnent son nom à ce vêtement » — est **à confirmer avant publication**, ou à assouplir en « ce sont les Anglais qui ont donné son nom à ce vêtement ».
